...
export default function CardSimulator() {
  ...
  useEffect(() => {
    ...
    if (backgroundImage) {
      ...
      // Draw IC chip (proportional placement)
      const chipImg = new Image();
      chipImg.src = orientation === "landscape" ? chipImage : chipImagePortrait;
      chipImg.width = 0;
      chipImg.height = 0;
      chipImg.onload = () => {
        // Corrected chip dimension calculations using actual IC chip proportions
  ...
  }, [orientation, image, scale, position, initialDisplaySize, backgroundImage, rotation]);
